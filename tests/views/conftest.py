...
def token(logged_in_user):
    return jwt.encode({"user_id": logged_in_user["user_id"], "is_master": logged_in_user["is_master"]},
...
def post(board, logged_in_user):
    return PostFactory.create(board=board, writer=logged_in_user.user_id)

@pytest.fixture
def comment(post, logged_in_user):
    return CommentFactory.create(post=post.id, writer=logged_in_user.user_id)