...
def test_comment_like_success(app, db, id_token, comment):
    token = id_token
    resp = app.post("/comment/like/" + str(comment.id), content_type="application/json", headers={"Token": token})
...
def test_comment_like_wrong_commentid(app, db, id_token, comment):
    token = id_token
    resp = app.post("/comment/like/" + str(comment.id) + "a", content_type="application/json", headers={"Token": token})
    data = json.loads(resp.data.decode("utf-8"))
    assert resp.status_code == 404
