...
def test_recomment_update_success(app, db, id_token, new_data, recomment):
    token = id_token
    resp = app.put("/recomment/" + str(recomment.id), data=json.dumps(new_data), content_type="application/json", headers={"Token": token})
...
def test_recomment_update_wrong_commentid(app, db, id_token, new_data, recomment):
    token = id_token
    resp = app.put("/recomment/" + str(recomment.id) + "a", data=json.dumps(new_data), content_type="application/json", headers={"Token": token})
    data = json.loads(resp.data.decode("utf-8"))
