...
def test_recomment_like_success(app, db, id_token, recomment):
    token = id_token
    resp = app.post("/recomment/like/?recomment_id=" + str(recomment.id), content_type="application/json", headers={"Token": token})
...
def test_recomment_like_wrong_commentid(app, db, id_token, recomment):
    token = id_token
    resp = app.post("/recomment/like/?recomment_id=" + str(recomment.id) + "a", content_type="application/json", headers={"Token": token})
    data = json.loads(resp.data.decode("utf-8"))
