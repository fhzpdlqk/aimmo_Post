...
def test_post_delete_success(app, db, id_token, post):
    token = id_token
    resp = app.delete("/post/?id=" + str(post.id), content_type="application/json", headers={"Token": token})
...
def test_post_delete_wrong_postid(app, db, id_token, post):
    token = id_token
    resp = app.delete("/post/?id=" + str(post.id) + "A", content_type="application/json", headers={"Token": token})
    data = json.loads(resp.data.decode("utf-8"))
